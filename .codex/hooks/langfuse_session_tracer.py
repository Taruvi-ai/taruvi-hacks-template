#!/usr/bin/env python3
"""Langfuse session tracer — ships everything the Codex agent DOES to Langfuse.

Wired in `.codex/config.toml` to three hook events, all pointing at this script:

    PreToolUse / PostToolUse / Stop  ->  langfuse_session_tracer.py

For every event, Codex passes a JSON payload on stdin. This script then:

1. Resolves which Langfuse trace the event belongs to, using the correlation
   file written by `langfuse_prompt_sync.py` (`.codex/log/codex_trace_correlation.json`).
   Matching order: session id -> payload conversation id -> most recent
   conversation within a 10-minute lookback -> deterministic id from session id.
   This stitching is what lands tool events in the SAME trace as the user's
   prompt, since tool payloads do not contain the prompt themselves.
2. Sanitizes inputs/outputs: redacts secret-shaped keys (token, password,
   authorization, ...) and values (bearer tokens, API keys, GitHub PATs,
   Langfuse keys), and bounds text length, list/dict size, and nesting depth.
3. Emits spans (`mcp_tool_call`, `bash_command`, `file_change`,
   `skill_selection`, `subagent_selection`, ...) to
   `{LANGFUSE_BASE_URL}/api/public/ingestion`, tagged with `app_slug`,
   `user_email`, `session_id`, `risk_level`, etc. for dashboard filtering.
4. Accumulates per-session state under `.codex/log/langfuse_sessions/` —
   tools used, files changed, detected skill/subagent, repeat counts.
5. On Stop, emits workflow scores (`agents_md_read`, `spec_writer_ran`,
   `qa_passed`, `final_quality`, ...) that grade whether the session followed
   this repo's intended workflow, then prints `{"continue":true}`.

Credentials come from `.env` / process env: LANGFUSE_PUBLIC_KEY,
LANGFUSE_SECRET_KEY, LANGFUSE_BASE_URL.

The hook FAILS OPEN by design: missing credentials, malformed payloads, and
network/Langfuse errors are logged to `.codex/log/langfuse_hook_debug.log`
and never block Codex execution.
"""
import hashlib
import json
import os
...
def project_root() -> Path:
    # Script lives at <project>/.codex/hooks/langfuse_session_tracer.py
...
def sanitize_full_prompt_text(value: str) -> str:
    return truncate_string(redact_string(value), limit=FULL_PROMPT_TEXT_LEN)
...
def extract_prompt_and_response(payload: dict[str, Any]) -> tuple[str | None, str | None]:
    ...
    if user_prompt:
        user_prompt = sanitize_full_prompt_text(user_prompt)
    if assistant_response:
        assistant_response = sanitize_full_prompt_text(assistant_response)
...
def main() -> int:
    ...
    try:
        ...
        user_prompt, assistant_response = extract_prompt_and_response(payload)
        prompt_preview = sanitize_preview_text(user_prompt) if user_prompt else None
        response_preview = (
            sanitize_preview_text(assistant_response) if assistant_response else None
        )
        user_prompt_available = bool(user_prompt)
        assistant_response_available = bool(assistant_response)
        ...
        if user_prompt_available:
            add_span(
                ...
                payload,
                common_meta,
                input_value={"prompt": user_prompt, "prompt_preview": prompt_preview},
                text_limit=FULL_PROMPT_TEXT_LEN,
            )
