#!/usr/bin/env python3
"""Langfuse prompt sync — ships what the user ASKED to Langfuse.

Wired in `.codex/config.toml` to run at SessionStart:

    SessionStart  ->  langfuse_prompt_sync.py

Codex tool-hook payloads never include the user's prompt, so this script
recovers prompts from Codex's local OTEL telemetry database instead:

1. Reads `~/.codex/logs_2.sqlite`, querying rows where
   `target = codex_otel.log_only` and `event.name = codex.user_prompt`,
   extracting prompt text, conversation/session/turn ids, timestamp,
   user email, model, and terminal info.
2. Redacts secret-shaped content (auth headers, cookies, bearer/basic
   tokens, Langfuse keys, API-key assignments) before anything leaves
   the machine.
3. Creates/updates a Langfuse trace named `codex_session` with a
   DETERMINISTIC trace id derived from the conversation id (same
   conversation -> same trace), and attaches a `user_prompt` span with
   the redacted text plus a compact preview.
4. Records synced row ids in `.codex/log/synced_prompt_ids.json` so
   re-runs are idempotent — delete that file only to force a full re-sync.
5. Writes `.codex/log/codex_trace_correlation.json`, mapping conversation
   ids to trace ids (+ prompt preview, user email, model, app slug).
   `langfuse_session_tracer.py` reads this file to attach later tool
   events to the same trace — it is the handoff between the two hooks.

Credentials come from `.env` / process env: LANGFUSE_PUBLIC_KEY,
LANGFUSE_SECRET_KEY, LANGFUSE_BASE_URL. Failures are logged to
`.codex/log/prompt_sync_debug.log` and never block the session.
"""
...
PROMPT_PAYLOAD_SCHEMA_VERSION = 2
...
def load_synced_ids(path: Path) -> set[int]:
    ...
    if isinstance(payload, dict):
        if payload.get("prompt_payload_schema_version") != PROMPT_PAYLOAD_SCHEMA_VERSION:
            return set()
...
def save_synced_ids(path: Path, ids: set[int]) -> None:
    ...
    payload = {
        "prompt_payload_schema_version": PROMPT_PAYLOAD_SCHEMA_VERSION,
...
def build_record(
    ...
    root: Path,
    *,
    span_event_type: str = "span-create",
) -> dict[str, Any] | None:
    ...
    prompt = redact_secrets(prompt_raw)
    prompt_preview = truncate_prompt_preview(prompt)
    ...
    span_body: dict[str, Any] = {
        ...
        "traceId": trace_id,
        "name": "user_prompt",
        "input": {"prompt": prompt, "prompt_preview": prompt_preview},
        "metadata": meta,
    }
    if span_event_type == "span-create":
        span_body["startTime"] = event_timestamp
        span_body["endTime"] = event_timestamp
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force-update",
        action="store_true",
        help="Send span-update events for matching prompt rows even if they are already synced.",
    )
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        for log_row_id, body in rows:
            dry_record = build_record(
                log_row_id,
                body,
                root,
                span_event_type="span-update" if args.force_update else "span-create",
            )
    ...
    for log_row_id, body in rows:
        ...
        if not args.force_update and log_row_id in synced_ids:
            skipped_existing += 1
            continue
